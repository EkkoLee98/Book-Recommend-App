<template>
	<div class="main-content" :style='{"padding":"20px 30px","background":"#f9fbfd","flexDirection":"column","display":"flex"}'>
			...
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"display":"flex"}' >
					<div :style='{"margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"none","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">账号</label>
					...
					<div :style='{"margin":"0 10px 0 0","display":"flex"}' class="select" label="图书分类" prop="tushufenlei">
						...
						<el-select clearable v-model="searchForm.tushufenlei" placeholder="请选择图书分类" multiple>
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"20px 0","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('yonghu','图书分类')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"rgb(106, 171, 156)","height":"40px"}'></span>
						图书分类
					</el-button>
				</el-row>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="tushufenleiChart2" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
//统计接口
    chartDialog2() {
      ...
      this.$nextTick(()=>{
        ...
        var tushufenleiChart2 = echarts.init(document.getElementById("tushufenleiChart2"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                tushufenleiChart2.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    tushufenleiChart2.resize();
                };
            }
        });
      })
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.tushufenlei&&this.searchForm.tushufenlei.length){
			  let tushufenleiArr = []
			  for(let i in this.searchForm.tushufenlei){
				  tushufenleiArr.push('%' + this.searchForm.tushufenlei[i] + '%')
			  }
            params['tushufenlei'] = tushufenleiArr.join(',')
          }
			this.$http({
				url: "yonghu/page",
				method: "get",
