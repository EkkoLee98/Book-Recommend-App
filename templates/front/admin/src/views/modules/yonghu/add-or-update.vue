		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="图书分类" prop="tushufenlei">
					<el-select :disabled="ro.tushufenlei" v-model="ruleForm.tushufenlei" placeholder="请选择图书分类" multiple filterable>
						<el-option
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.tushufenlei = this.ruleForm.tushufenlei.split(",");
        } else {
          this.$message.error(data.msg);
        }
      });
      ...
      //更新跨表属性
       ...
       if(this.type=='cross'){
		...
		this.$refs["ruleForm"].validate(valid => {
			if (valid) {
				this.ruleForm.tushufenlei = this.ruleForm.tushufenlei.join(",");
