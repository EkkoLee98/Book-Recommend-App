...
<div class="center-preview" :style='{"width":"80%","margin":"10px auto","position":"relative","background":"none"}'>
	...
	<div class="info" :style='{"padding":"50px 50px 50px 280px","boxShadow":"none","margin":"0 auto 20px","borderRadius":"0","flexWrap":"wrap","background":"#ffffff","display":"flex","width":"100%","position":"relative","height":"260px"}'>
		...
		<div :style='{"padding":"0 0","borderColor":"#efefef","flexWrap":"wrap","borderWidth":"0","flexDirection":"column","display":"flex","width":"calc(100% / 3)","lineHeight":"40px","borderStyle":"solid","height":"50%"}' v-if="userTableName=='yonghu'">
			...
			<div :style='{"width":"calc(100% - 75px)","fontSize":"16px","color":"#000","height":"50%"}'>昵称</div>
			<div :style='{"width":"calc(100% - 75px)","fontSize":"15px","color":"#333","textAlign":"left","flex":1,"height":"50%"}'>{{sessionForm.nicheng}}</div>
		</div>
		<div :style='{"padding":"0 0","borderColor":"#efefef","flexWrap":"wrap","borderWidth":"0","flexDirection":"column","display":"flex","width":"calc(100% / 3)","lineHeight":"40px","borderStyle":"solid","height":"50%"}' v-if="userTableName=='yonghu'">
		...
		<div :style='{"padding":"0 0","borderColor":"#efefef","flexWrap":"wrap","borderWidth":"0","flexDirection":"column","display":"flex","width":"calc(100% / 3)","lineHeight":"40px","borderStyle":"solid","height":"50%"}' v-if="userTableName=='yonghu'">
			...
			<div :style='{"width":"calc(100% - 75px)","fontSize":"16px","color":"#000","height":"50%"}'>图书分类</div>
			<div :style='{"width":"calc(100% - 75px)","fontSize":"14px","color":"#333","textAlign":"left","height":"50%"}'>{{sessionForm.tushufenlei}}</div>
		</div>
    ...
    <el-tabs tab-position="left" :style='{"width":"100%","flexWrap":"wrap","background":"#fff","justifyContent":"center","display":"flex"}' @tab-click="handleClick">
      <el-tab-pane label="个人中心">
        <el-form class="center-preview-pv" ref="sessionForm" :model="sessionForm" :rules="rules" label-width="200px">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='yonghu'" label="昵称" prop="nicheng">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='yonghu'" label="图书分类" prop="tushufenlei">
            <el-select v-model="sessionForm.tushufenleiArray" placeholder="请选择图书分类" multiple @change="yonghutushufenleiMulChange" >
              <el-option v-for="(item, index) in dynamicProp.tushufenlei" :key="index" :label="item" :value="item"></el-option>
            </el-select>
          </el-form-item>
...
<script>
  ...
  export default {
    ...
    created() {
		...
		if ('yonghu' == this.userTableName) {
			this.$set(this.sessionForm, 'tushufenlei', null);
		}
		if ('yonghu' == this.userTableName) {
    ...
    methods: {
      init() {
        ...
        if ('yonghu' == this.userTableName) {
          this.$http.get('option/tushufenlei/tushufenlei', {emulateJSON: true}).then(res => {
            if (res.data.code == 0) {
              this.dynamicProp.tushufenlei = res.data.data;
			  this.$forceUpdate()
            }
          });
        }
	  ...
	  setSession(){
        ...
        yonghutushufenleiMulChange(e){
            this.sessionForm.tushufenlei = this.sessionForm.tushufenleiArray.join(',')
        },
      onSubmit(formName) {
