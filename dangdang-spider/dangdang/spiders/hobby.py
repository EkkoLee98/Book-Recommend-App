...
class HobbySpider(scrapy.Spider):
    name = 'hobby'
    allowed_domains = ['search.dangdang.com']
    start_urls = ('http://search.dangdang.com/?key=休闲/爱好&page_index=1',)
    p = 1
    ...
    def parse(self, response):
        ...
        self.p += 1
        if self.p < 2:
            next_url = 'http://search.dangdang.com/?key=休闲/爱好&page_index=' + \
                str(self.p)
