...
def yonghu_update(request):
    ...
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
        if yonghu.objects.filter(id__gt=req_dict['id'],zhanghao = req_dict['zhanghao']).count()>0:
            msg['code'] = crud_error_code
            msg['msg'] = "账号已存在"
            return JsonResponse(msg)
            ...
            try:
                for row in range(1, rows):
                    ...
                    if '.0' in str(row_values[2]):
                        req_dict['nicheng'] = str(row_values[2]).split('.')[0]
                    elif str(row_values[2]) != '':
                        req_dict['nicheng'] = row_values[2]
                    else:
                        req_dict['nicheng'] = None
